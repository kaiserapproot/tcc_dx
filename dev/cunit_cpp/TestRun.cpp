...
//int ssnprintf(char* buf, size_t size, const char* fmt, ...);
/*=================================================================
...

{
  ...
  if (NULL != result) {
      _snprintf(result, len, "%*s%*s%*s%*s%*s%*s%*s\n"   /* if you change this, be sure  */
            ...
            width[4], pRunSummary->nSuitesRun - pRunSummary->nSuitesFailed,
            ...
            width[6], _("なし"),
            width[7], _("合計時間 = "), CU_get_elapsed_time(),  /* makes sure time is updated */
            width[8], _(" seconds")
            );
     result[len-1] = '\0';
  }
