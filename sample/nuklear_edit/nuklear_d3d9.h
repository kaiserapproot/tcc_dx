...
#endif
#ifndef CP_UTF8
#define CP_UTF8 65001  /* UTF-8のコードページ番号 */
#endif
#ifndef S_PRESENT_OCCLUDED
#define S_PRESENT_OCCLUDED ((HRESULT)0x00262307L)
#endif

/* VC++6.0用のニーズに合わせてマクロを定義 */
#define NK_ALIGNOF(type) (__alignof(type))
#endif
...
nk_d3d9_create_font_texture()
{
    ...
    for (y = 0; y < h; y++) {
        src = (char *)image + y * w * 4;
        dst = (char *)locked.pBits + y * locked.Pitch;
        memcpy(dst, src, w * 4);
    }
...
nk_d3d9_handle_event(HWND wnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_CHAR:
        if (wparam >= 32)
        {
                ...
                /* diagnostic: always log unicode input to ./nuklear_edit_diag.log so Release runs can be analyzed */
                {
                    char dbgbuf[256];
                    int cursor = -1;
                    if (d3d9.ctx.current) cursor = d3d9.ctx.current->edit.cursor;
                    sprintf_s(dbgbuf, sizeof(dbgbuf), "[diag] nk_d3d9 WM_CHAR rune=0x%X ctx_current=%p edit_cursor=%d\n",
                        (unsigned)wparam, (void*)d3d9.ctx.current, cursor);
                    OutputDebugStringA(dbgbuf);
                    /* append to current dir logfile */
                    {
                        HANDLE h = CreateFileA("nuklear_edit_diag.log", GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
                        if (h != INVALID_HANDLE_VALUE) {
                            DWORD written = 0;
                            SetFilePointer(h, 0, NULL, FILE_END);
                            WriteFile(h, dbgbuf, (DWORD)strlen(dbgbuf), &written, NULL);
                            CloseHandle(h);
                        }
                    }
                }
    ...
    case WM_LBUTTONDOWN:
        {
            ...
            nk_input_button(&d3d9.ctx, NK_BUTTON_LEFT, x, y, 1);
            SetCapture(wnd);
            
            current_time = GetTickCount() / 1000.0;
            if ((current_time - d3d9.last_button_click) < 0.2) {
                nk_input_button(&d3d9.ctx, NK_BUTTON_DOUBLE, x, y, 1);
            }
            d3d9.last_button_click = current_time;
            return 1;
        }
    ...
    case WM_LBUTTONUP:
        {
            ...
            nk_input_button(&d3d9.ctx, NK_BUTTON_DOUBLE, x, y, 0);
            nk_input_button(&d3d9.ctx, NK_BUTTON_LEFT, x, y, 0);
            ReleaseCapture();
            return 1;
        }
...
nk_d3d9_clipboard_paste(nk_handle usr, struct nk_text_edit *edit)
{
    ...
    (void)usr;
    /* ensure format available and clipboard opened */
    ...
    /* get real wide-char length (exclude terminating NUL) */
    {
        ...
        /* デバッグ: クリップボードの生データを16進表示 */
        {
            unsigned char *raw = (unsigned char*)wstr;
            unsigned int rawlen = (unsigned int)(wlen * sizeof(wchar_t));
            unsigned int i;
            printf("[nk_d3d9_clipboard_paste] clipboard-raw (%u bytes):", rawlen);
            for (i = 0; i < rawlen; ++i) printf(" %02X", raw[i]);
            printf("\n");
        }
        ...
        if (utf8size) {
            ...
            if (utf8) {
                ...
                utf8[utf8size] = '\0';
                /* デバッグ: 変換後の UTF-8 バイト列を表示 */
                {
                    unsigned int i;
                    printf("[nk_d3d9_clipboard_paste] clipboard-utf8 (%d bytes):", utf8size);
                    for (i = 0; i < (unsigned int)utf8size; ++i) printf(" %02X", (unsigned char)utf8[i]);
                    printf("\n");
                }
...
nk_d3d9_clipboard_copy(nk_handle usr, const char *text, int len)
{
    ...
    /* len passed by nuklear might be character count rather than byte count for UTF-8.
     * Fall back to strlen(text) when len is smaller than actual byte length to avoid
     * cutting UTF-8 sequences and producing U+FFFD replacement characters. */
    {
        size_t text_bytes = strlen(text);
        if (len <= 0 || (size_t)len < text_bytes) {
            len = (int)text_bytes;
        }
    }
