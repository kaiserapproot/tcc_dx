...
{
	char buf[180];
	sprintf(buf, "期待値:<%d> に対して実際の値:<%d>", expected, actual);
...
{
	char buf[180];
	sprintf(buf, "期待値:<%ld> に対して実際の値:<%ld>", expected, actual);
	return cu_String(buf);
}
...
{
	char buf[180];
	sprintf(buf, "期待値:<%f> に対して実際の値:<%f>", expected, actual);
	return cu_String(buf);
}
