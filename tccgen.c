...
static Sym* func_class; /* 生成中の関数が C++ メソッドの場合、所属クラスの Sym（それ以外は NULL） */
static int cpp_extern_c_level; /* extern "C" { } の入れ子深さ */

/* C++ オーバーロード集合: 基底名トークン → 実シンボル名トークンの対応表。
   同名で異なるシグネチャの宣言が現れたときだけ登録される（初出は素の名前を保つ）ため、
   通常の C 関数・main・extern "C" は一切影響を受けない */
...
/* C++ メソッドの内部シンボル名トークンを返す: __<Class>__<method>
   （簡易マングリング。オーバーロードは未対応） */
...
/* クラスとその基底クラスを順にたどってメソッドを探す
   （単一継承・基底はオフセット 0 のため this 変換は不要） */
...
/* クラスとその基底クラスを順にたどってマングル済みシンボル
   （static メンバ変数・メソッドを含む任意の種別）を探す */
...
/* シンボルが「this を第1引数に持つインスタンスメソッド」なら 1 を返す */
...
/* 関数型のシグネチャを付与した名前トークン `<base>__O<codes>` を返す。
   第 1 引数が this の場合はシグネチャから除外する */
...
/* 宣言時に使う実シンボル名を返す。同名で異なるシグネチャの宣言が現れた場合のみ
   マングル名を割り当て、オーバーロード集合に登録する */
static int cpp_overload_name(int base, CType* ftype)
{
    ...
    if (!cpp_ovl_has(base)) {
        ...
        if (!s || (s->type.t & VT_BTYPE) != VT_FUNC)
            return base; /* 初出: 素の名前を使う */
        if (is_compatible_types(&s->type, ftype))
            return base; /* 同一シグネチャの再宣言・定義 */
        cpp_ovl_add(base, base); /* 既存の宣言を集合に取り込む */
    }
    else {
        /* 既存の候補と同一シグネチャならその名前を再利用（宣言 → 定義） */
        for (i = 0; i < cpp_nb_ovl; i++) {
            if (cpp_ovl[i].base_tok != base)
                continue;
...
static int cpp_arg_score(SValue* sv, CType* pt)
{
    ...
    if (is_float(at->t) == is_float(pt->t))
        return 2; /* 整数同士 / 浮動小数同士 */
    return 1;     /* 整数 ↔ 浮動小数 */
}

/* 実引数から最適なオーバーロードを選ぶ。args は vstack 上の先頭引数 */
static Sym* cpp_select_overload(int base, int nb_args, SValue* args, int has_this)
{
    ...
    if (!best) {
        if (found)
            tcc_error("'%s' の呼び出しに一致するオーバーロードがありません",
                get_tok_str(base, NULL));
        tcc_error("'%s' は宣言されていません", get_tok_str(base, NULL));
    }
    if (ambiguous)
        tcc_error("'%s' の呼び出しが曖昧です", get_tok_str(base, NULL));
    return best;
}

/* オーバーロードされた関数呼び出しを生成する。
   呼び出し時 tok == '('。has_this なら vtop に this が積まれていること */
static void cpp_call_overload(int base, int has_this)
{
    ...
    if ((fs->type.ref->type.t & VT_BTYPE) == VT_STRUCT)
        tcc_error("オーバーロードされた関数は構造体を値で返せません");

    /* 引数の型変換: nb_args 回転すると元の順序に戻る */
    sa = fs->type.ref->next;
    if (has_this && sa)
        sa = sa->next;
    for (i = 0; i < nb_args; i++) {
...
/* クラス階層をたどってメソッド名のオーバーロード集合の基底トークンを返す。
   オーバーロードされていなければ 0 */
...
/* トークンが型の開始になり得るなら 1（`Point p(3,4)` と関数宣言の判別用） */
...
/* '(' の中身（対応する ')' の手前まで）を dst にコピーする。
   呼び出し時点で tok は '(' の次のトークン。終了時 tok == ')' */
...
/* 基底クラス ctor/dtor 呼び出し文 `__Base__x((void*)this <,args>);` の
   冒頭部分を dst に合成する（args と閉じは呼び出し側で追加） */
...
/* コンストラクタ/デストラクタの本体パースと登録。
   呼び出し時点で tok == '(' であること。mtok は __ctor / __dtor のトークン。
   初期化子リスト `: Base(args), member(expr)` と基底 ctor/dtor の
   自動連鎖に対応する */
static void cpp_parse_ctor_dtor(Sym* s, int mtok)
{
    ...
    if (is_ctor && tok == ':') {
        ...
        do {
            ...
            if (s->base_class && name == (s->base_class->v & ~SYM_STRUCT)) {
                /* 基底クラス ctor: __Base____ctor((void*)this, args); */
                if (sbase)
                    tcc_error("基底クラス初期化子が重複しています");
                sbase = tok_str_alloc();
    ...
    if (tok != '{')
        tcc_error("コンストラクタ/デストラクタには本体が必要です");
    ...
    /* 初期化子リストに基底が無くても、基底に ctor があれば暗黙にデフォルト呼び出し */
    ...
    if (!sbase && !smem && !bdtor) {
        ...
        /* 合成本体: { <基底ctor> <メンバ初期化> <元の本体> <基底dtor> } */
        body = tok_str_alloc();
        tok_str_add(body, '{');
        if (sbase)
...
do_decl:
    ...
    if (u == VT_STRUCT && tok == ':' && tcc_state->cplusplus) {
        /* C++ 単一継承: class Derived : [public] Base { ... } */
        Sym* bs;
        next();
        ...
        if (!bs || (bs->type.t & VT_BTYPE) != VT_STRUCT || bs->c < 0)
            tcc_error("基底クラス '%s' が未定義です", get_tok_str(tok, NULL));
        s->base_class = bs;
        next();
        if (tok == ',')
            tcc_error("多重継承は未対応です");
        if (tok != '{')
            expect("クラス本体 '{'");
    }
    ...
    if (tok == '{') {
        ...
        enum_done:
            ...
            flexible = 0;
            if (s->base_class) {
                /* 基底クラスをオフセット 0 の無名フィールドとして埋め込む。
                   find_field() の無名構造体再帰により基底メンバへの
                   アクセスが自動的に解決される（単一継承なので
                   Derived* → Base* の変換は恒等） */
                CType bt1;
            ...
            while (tok != '}') {
                ...
                if (tcc_state->cplusplus && u == VT_STRUCT && tok >= TOK_UIDENT
                    ...
                    next();
                    if (tok != '(') {
                        unget_tok(cname); /* typedef 名などの通常メンバ宣言 → 巻き戻し */
                    }
                    else {
                ...
                while (1) {
                    ...
                    if (tok != ':') {
                        ...
                        if (tcc_state->cplusplus
                            ...
                            && u == VT_STRUCT && v) {
                            /* C++ static データメンバ: グローバル変数
                               __<Class>__<v> の extern 宣言として登録する。
                               定義は「型 Class::v = 初期値;」で行う */
                            CType stype;
                            stype = type1;
                        ...
                        if (tcc_state->cplusplus
                            ...
                            /* C++ メンバ関数（struct/class 内）の最小対応 */
                            if (tok == '{') {
                                /* インライン本体: 第1引数に this を挿入した
                                   static inline 関数 __<Class>__<method> として保存。
                                   参照された場合のみ gen_inline_functions() が
                                   遅延生成する（フィールドには積まない）。
                                   static メソッドは this を挿入しない */
                                CType mtype, ct;
                                Sym* msym, * pthis;
                                struct InlineFunc* fn;
...
static int parse_btype(CType* type, AttributeDef* ad, int ignore_label)
{
    ...
    while (1) {
        ...
        case TOK_CLASS:
            if (!tcc_state->cplusplus)
                goto default_ident; /* C モードでは通常の識別子 */
            /* fallthrough */
        case TOK_STRUCT:
...
static int post_type(CType* type, AttributeDef* ad, int storage, int td)
{
    ...
    if (tok == '(') {
        /* C++: ctor を持つクラス型変数の宣言 `Point p(3,4);` では
           '(' はコンストラクタ引数なので、関数宣言として消費しない。
           1 トークン先読みして型が現れれば関数宣言とみなす */
        if (tcc_state->cplusplus
...
ST_FUNC void unary(void)
{
    ...
    int mcall = 0; /* C++: [this][func] が積まれておりメソッド呼び出し待ちなら 1 */
    CType type;
    Sym* s;
    AttributeDef ad;
...
tok_next:
    ...
    tok_identifier:
        ...
        if (tcc_state->cplusplus && tok == '(' && cpp_ovl_has(t)) {
            /* C++ オーバーロードされた自由関数の呼び出し */
            cpp_call_overload(t, 0);
            break;
        }
        if (!s && tok == ':' && tcc_state->cplusplus) {
            /* C++ Class::member（static メンバ / static メソッド） */
            Sym* cls = struct_find(t);
            if (cls && (cls->type.t & VT_BTYPE) == VT_STRUCT) {
                next();
                if (tok != ':') {
                    unget_tok(':'); /* 三項演算子などの単独 ':' → 巻き戻し */
                }
                else {
                    next();
                    if (tok < TOK_UIDENT)
                    ...
                    if (cpp_is_instance_method(s))
                        tcc_error("インスタンスメソッド '%s::%s' はオブジェクト経由で呼び出してください",
                            get_tok_str(t, NULL), get_tok_str(tok, NULL));
                    next();
                }
            }
        ...
        if (func_class && (!s || !s->sym_scope)) {
            /* C++ メソッド本体内: 未解決またはグローバルの識別子は
               クラススコープを優先して解決する（ローカル/引数が最優先、
               クラスメンバ、グローバルの順） */
            int cumofs;
            CType ct;
            Sym* f, * sthis, * ms;
            ...
            if (ms) {
                if (cpp_is_instance_method(ms)) {
                    ...
                    /* static メソッド / static データメンバ: 通常シンボルとして解決 */
                    s = ms;
                }
            }
        }
    ...
    while (1) {
        if (tok == TOK_INC || tok == TOK_DEC) {
            ...
            if ((vtop->type.t & VT_BTYPE) == VT_STRUCT && vtop->type.ref
                ...
                if (ms && (ms->type.t & VT_BTYPE) == VT_FUNC) {
                    ...
                    if (cpp_is_instance_method(ms)) {
                        ...
                        /* static メソッド: オブジェクト式は捨てて通常呼び出し */
                        vpop();
                        vpushsym(&ms->type, ms);
                    }
                    next();
                    if (tok != '(')
                        tcc_error("メソッド '%s' は呼び出し以外に使用できません",
            ...
            if (mcall && sa)
                sa = sa->next; /* this は既に積んであるため型検査をスキップ */
            nb_args = regsize = 0;
            ret.r2 = VT_CONST;
...
static void gen_function(Sym* sym)
{
    ...
    /* C++ メソッドなら所属クラスを記録
       （本体内の暗黙メンバアクセス this->x やクラススコープ解決に使う。
        static メソッドにも設定される） */
...
static int decl(int l)
{
    ...
    CType type, btype;
    Sym* sym;
    Sym* cxctor, * cxdtor; /* C++: ローカル class 変数の ctor/dtor */
...
#endif
            if ((type.t & VT_BTYPE) == VT_FUNC) {
                /* C++: 同名で異なるシグネチャならオーバーロード名を割り当てる。
                   extern "C" 内と初出の宣言は素の名前のまま */
                ...
                found:
                    ...
                    if (tcc_state->cplusplus && l == VT_LOCAL
                        ...
                        if (cxdtor && !ad.cleanup_func)
                            ad.cleanup_func = cxdtor; /* スコープ離脱時に自動破棄 */
                    }
                    has_init = (tok == '=');
                    ...
                    if (((type.t & VT_EXTERN) && (!has_init || l != VT_CONST))
                        ...
                        if (cxctor && !has_init) {
                            ...
                            tok_str_add_tok(cstr); /* 現在のトークンも積む */
                            tok_str_add(cstr, 0);
                            begin_macro(cstr, 1);
                            next();
                            expr_eq(); /* ',' 演算子を含まない式として評価 */
                            vpop();
                        }
                    }
