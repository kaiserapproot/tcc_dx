...
| 構築・破棄 | コンストラクタ（初期化子リスト含む）、デストラクタ、ローカルのブロック終了時 dtor、グローバルの自動 ctor/dtor（`.init_array` / `.fini_array`）、暗黙の基底 ctor / 自動基底 dtor、**`const` メンバの mem-initializer 初期化**、**暗黙メンバ ctor/dtor**（mem-init が挙げないクラス型データメンバの黙示構築・逆順破棄）、**変換 ctor の暗黙適用**（G-CONV。`return` / mem-init / 代入 / 引数で 1 段のみ）（いずれも 2026-08-23） |
...
| # | 機能 | 最小再現 | 期待 | 実際 |
|---|---|---|---|---|
| A | **コピー初期化 `T b = a;` でコピーコンストラクタが呼ばれない** | `P a; P b = a;` | `b.v == 2` | **`b.v == 1`**（memcpy のまま。2026-08-29 再実測で未解消。**直接初期化 `P b(a);` は正しくコピー ctor を呼ぶ** — 同日実測で `b.v == 101`） |
...
> **A の適用範囲**（2026-08-29 実測で切り分け）: 影響を受けるのは
> **コピー初期化 `T b = a;` だけ**である。同じ意味の**直接初期化 `T b(a);` は
> コピー ctor を正しく解決して呼ぶ**（FEAT-4B の `Foo f(args);` 経路にオーバーロード
> 解決が乗っているため）。したがって**回避策は `T b(a);` と書くこと**。
> ユーザー定義 `operator=` による `b = a;`（宣言と分けた代入）も正しく呼ばれる
> （FEAT-6A-ext2。同日実測）。
> **`new T(obj)` は BUG-46 / BUG-47 で対応済み**（2026-08-24）で、
> フラットなバイトコピーではなくメンバワイズの暗黙コピー（ヒープバッファを持つ
> メンバはそのクラスのコピー用 ctor で再構築、既定引数も補完）を行う。
> また**引数渡し・`return`・代入**でクラス型が要求される位置では、G-CONV の
> 変換 ctor 適用（1 段）が働く。残っているのは宣言初期化子の経路のみ。
...
5. **コピーは `P b(a);` と直接初期化で書く**。`P b = a;`（コピー初期化）だけは
   コピーコンストラクタを呼ばず memcpy になる（§3.2 A）。`P b(a);` /
   `P b; b = a;`（`operator=`）/ `new P(a)` はいずれも正しく動く。
