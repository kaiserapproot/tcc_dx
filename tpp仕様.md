...
| E | **同名オーバーロードの前方参照が解決されない（BUG-30）** | クラス内に `f(int)` / `f(const char*)` を宣言し、**定義より前**に `s.f(1)` を呼ぶ | `f(int)` | **最初に宣言した方**（引数の数が合わなければ `関数への引数が多すぎます` で停止、合えば黙って誤呼び出し）。`.h` 宣言＋`.cpp` 定義の通常構成が該当。回避: 呼び出しより前に定義を置く |
...
| BUG-30 | 同名オーバーロードの**前方参照**が解決されない（定義済みのものしか候補にならない） | 未修正（§3.2）。ガイド G-OVL で対応予定 |
