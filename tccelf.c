...
ST_FUNC void tcc_add_cpp_tls_runtime(TCCState *s1)
{
    ...
    cstr_cat(&cstr,
        ...
        "    TCC_CPP_TLS_N6_DELETE_BLOCKED_LIVE_TLS,\n"
        "    TCC_CPP_TLS_N6_POST_DELETE_TLS_TOUCH,\n"
        ...
        // N6-07-04: runtime-wide live TCB authority + tcc_delete tombstone.
        "static volatile long tcc_cpp_tls_n6_delete_state;\n"
        "static long tcc_cpp_tls_n6_live_tcb_read(void)\n"
        "{\n"
        // N6-04B TCB_ALLOC/TCB_FREE is the runtime-wide live TCB authority.
        "    return tcc_cpp_tls_n6_stat[TCC_CPP_TLS_N6_TCB_ALLOC]\n"
        "         - tcc_cpp_tls_n6_stat[TCC_CPP_TLS_N6_TCB_FREE];\n"
        "}\n"
        ...
        "            abort();\n"
        "        }\n"
        "        if (tcc_cpp_tls_n6_delete_state != 0) {\n"
        "            tcc_cpp_tls_n6_count(TCC_CPP_TLS_N6_POST_DELETE_TLS_TOUCH);\n"
        "            abort();\n"
        "        }\n"
        ...
        "}\n"
        // N6-07-04: libtcc tcc_delete gate (fail-closed; run_ptr free only after pass).
        "long __cdecl __tcc_cpp_tls_n6_live_tcb_count_read(void)\n"
        "{\n"
        "    return tcc_cpp_tls_n6_live_tcb_read();\n"
        "}\n"
        "int __cdecl __tcc_cpp_tls_n6_delete_state_read(void)\n"
        "{\n"
        "    return (int)tcc_cpp_tls_n6_delete_state;\n"
        "}\n"
        "void __cdecl __tcc_cpp_tls_n6_tcc_delete_gate(void)\n"
        "{\n"
        "    long live;\n"
        "    tcc_cpp_tls_n6_delete_state = 1;\n"
        "    live = tcc_cpp_tls_n6_live_tcb_read();\n"
        "    if (live > 0) {\n"
        "        tcc_cpp_tls_n6_count(TCC_CPP_TLS_N6_DELETE_BLOCKED_LIVE_TLS);\n"
        "        abort();\n"
        "    }\n"
        "}\n"
    ...
    if (memory_exec) {
        ...
        "}\n"
        // N6-06A2: -run worker normal-return cleanup via thread-entry trampoline.
        // PE TLS callback must stay NOOP after wrapper drain (slot NULL, TCB freed).
        // Resolve real APIs lazily; never rewrite user source identifiers.
        "typedef DWORD (WINAPI *tcc_n6_k32_create_thread_fn)(\n"
        "    LPSECURITY_ATTRIBUTES, SIZE_T, LPTHREAD_START_ROUTINE,\n"
        "    LPVOID, DWORD, LPDWORD);\n"
        "typedef struct tcc_n6_ct_wrap_ctx {\n"
        "    LPTHREAD_START_ROUTINE proc;\n"
        "    LPVOID arg;\n"
        "} tcc_n6_ct_wrap_ctx;\n"
        "typedef unsigned (__stdcall *tcc_n6_btx_user_fn)(void *);\n"
        "typedef struct tcc_n6_btx_wrap_ctx {\n"
        "    tcc_n6_btx_user_fn proc;\n"
        "    void *arg;\n"
        "} tcc_n6_btx_wrap_ctx;\n"
        "typedef uintptr_t (__cdecl *tcc_n6_crt_beginthreadex_fn)(\n"
        "    void *, unsigned, tcc_n6_btx_user_fn, void *, unsigned, unsigned *);\n"
        "static tcc_n6_k32_create_thread_fn tcc_n6_real_create_thread;\n"
        "static tcc_n6_crt_beginthreadex_fn tcc_n6_real_beginthreadex;\n"
        "static void tcc_n6_resolve_create_thread(void)\n"
        "{\n"
        "    HMODULE h;\n"
        "    if (tcc_n6_real_create_thread)\n"
        "        return;\n"
        "    h = GetModuleHandleA(\"kernel32.dll\");\n"
        "    if (!h)\n"
        "        abort();\n"
        "    tcc_n6_real_create_thread = (tcc_n6_k32_create_thread_fn)\n"
        "        GetProcAddress(h, \"CreateThread\");\n"
        "    if (!tcc_n6_real_create_thread)\n"
        "        abort();\n"
        "}\n"
        "static void tcc_n6_resolve_beginthreadex(void)\n"
        "{\n"
        "    HMODULE h;\n"
        "    if (tcc_n6_real_beginthreadex)\n"
        "        return;\n"
        "    h = GetModuleHandleA(\"msvcrt.dll\");\n"
        "    if (!h)\n"
        "        abort();\n"
        "    tcc_n6_real_beginthreadex = (tcc_n6_crt_beginthreadex_fn)\n"
        "        GetProcAddress(h, \"_beginthreadex\");\n"
        "    if (!tcc_n6_real_beginthreadex)\n"
        "        abort();\n"
        "}\n"
        "static DWORD WINAPI tcc_n6_create_thread_trampoline(LPVOID param)\n"
        "{\n"
        "    tcc_n6_ct_wrap_ctx ctx;\n"
        "    DWORD ret;\n"
        "    ctx = *(tcc_n6_ct_wrap_ctx *)param;\n"
        "    HeapFree(GetProcessHeap(), 0, param);\n"
        "    ret = ctx.proc(ctx.arg);\n"
        "    __tcc_cpp_tls_n6_cleanup_current_thread();\n"
        "    return ret;\n"
        "}\n"
        "static unsigned __stdcall tcc_n6_beginthreadex_trampoline(void *param)\n"
        "{\n"
        "    tcc_n6_btx_wrap_ctx ctx;\n"
        "    unsigned ret;\n"
        "    ctx = *(tcc_n6_btx_wrap_ctx *)param;\n"
        "    HeapFree(GetProcessHeap(), 0, param);\n"
        "    ret = ctx.proc(ctx.arg);\n"
        "    __tcc_cpp_tls_n6_cleanup_current_thread();\n"
        "    return ret;\n"
        ...
        "    tcc_n6_ct_wrap_ctx *ctx;\n"
        "    if (!lpStartAddress) {\n"
        "        tcc_n6_resolve_create_thread();\n"
        "        return tcc_n6_real_create_thread(lpThreadAttributes, dwStackSize,\n"
        "            lpStartAddress, lpParameter, dwCreationFlags, lpThreadId);\n"
        "    }\n"
        "    ctx = (tcc_n6_ct_wrap_ctx *)HeapAlloc(GetProcessHeap(), 0,\n"
        "        sizeof(tcc_n6_ct_wrap_ctx));\n"
        "    if (!ctx)\n"
        "        return 0;\n"
        ...
        "    tcc_n6_resolve_create_thread();\n"
        "    return tcc_n6_real_create_thread(lpThreadAttributes, dwStackSize,\n"
        "        tcc_n6_create_thread_trampoline, ctx, dwCreationFlags, lpThreadId);\n"
        "}\n"
        "uintptr_t __cdecl _beginthreadex(void *security, unsigned stack_size,\n"
        "    tcc_n6_btx_user_fn start_address, void *arglist, unsigned initflag,\n"
        "    unsigned *thrdaddr)\n"
        "{\n"
        "    tcc_n6_btx_wrap_ctx *ctx;\n"
        "    if (!start_address) {\n"
        "        tcc_n6_resolve_beginthreadex();\n"
        "        return tcc_n6_real_beginthreadex(security, stack_size,\n"
        "            start_address, arglist, initflag, thrdaddr);\n"
        "    }\n"
        "    ctx = (tcc_n6_btx_wrap_ctx *)HeapAlloc(GetProcessHeap(), 0,\n"
        "        sizeof(tcc_n6_btx_wrap_ctx));\n"
        "    if (!ctx)\n"
        "        return 0;\n"
        "    ctx->proc = start_address;\n"
        "    ctx->arg = arglist;\n"
        "    tcc_n6_resolve_beginthreadex();\n"
        "    return tcc_n6_real_beginthreadex(security, stack_size,\n"
        "        tcc_n6_beginthreadex_trampoline, ctx, initflag, thrdaddr);\n"
        "}\n"
        // N6-07-02: -run has no loader TLS directory; ExitThread must drain here
        // (CreateThread return-path cleanup never runs after ExitThread).
        "typedef VOID (WINAPI *tcc_n6_k32_exit_thread_fn)(DWORD);\n"
        "typedef void (__cdecl *tcc_n6_crt_endthreadex_fn)(unsigned);\n"
        "static tcc_n6_k32_exit_thread_fn tcc_n6_real_exit_thread;\n"
        "static tcc_n6_crt_endthreadex_fn tcc_n6_real_endthreadex;\n"
        "static void tcc_n6_resolve_exit_thread(void)\n"
        "{\n"
        "    HMODULE h;\n"
        "    if (tcc_n6_real_exit_thread)\n"
        "        return;\n"
        "    h = GetModuleHandleA(\"kernel32.dll\");\n"
        "    if (!h)\n"
        "        abort();\n"
        "    tcc_n6_real_exit_thread = (tcc_n6_k32_exit_thread_fn)\n"
        "        GetProcAddress(h, \"ExitThread\");\n"
        "    if (!tcc_n6_real_exit_thread)\n"
        "        abort();\n"
        "}\n"
        "static void tcc_n6_resolve_endthreadex(void)\n"
        "{\n"
        "    HMODULE h;\n"
        "    if (tcc_n6_real_endthreadex)\n"
        "        return;\n"
        "    h = GetModuleHandleA(\"msvcrt.dll\");\n"
        "    if (!h)\n"
        "        abort();\n"
        "    tcc_n6_real_endthreadex = (tcc_n6_crt_endthreadex_fn)\n"
        "        GetProcAddress(h, \"_endthreadex\");\n"
        "    if (!tcc_n6_real_endthreadex)\n"
        "        abort();\n"
        "}\n"
        ...
        "{\n"
        "    __tcc_cpp_tls_n6_cleanup_current_thread();\n"
        "    tcc_n6_resolve_exit_thread();\n"
        "    tcc_n6_real_exit_thread(dwExitCode);\n"
        "}\n"
        "void __cdecl _endthreadex(unsigned ret)\n"
        "{\n"
        "    __tcc_cpp_tls_n6_cleanup_current_thread();\n"
        "    tcc_n6_resolve_endthreadex();\n"
        "    tcc_n6_real_endthreadex(ret);\n"
        "}\n",
        -1);
    } else {
        cstr_cat(&cstr,
    ...
    cstr_cat(&cstr,
        ...
        "        }\n"
        "    }\n"
        // N6-07-04: new first-touch only; existing constructed objects stay reachable.
        "    if (tcc_cpp_tls_n6_delete_state != 0) {\n"
        "        tcc_cpp_tls_n6_count(TCC_CPP_TLS_N6_POST_DELETE_TLS_TOUCH);\n"
        "        abort();\n"
        "    }\n"
