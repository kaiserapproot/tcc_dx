...
> **進捗（2026-08-25）**: `all_test_tpp.cpp`（N=17）で BUG-43〜48 を修正し、
> `sample/cppunit/build_cppunit.bat` の機械ゲートを実装済み。17 個の marker、
> `TESTS:17` / `FAILURES:0` / `ERRORS:0` / exit 0、baseline 由来31ファイルの
> 無改変を確認している。`dev/test/run_all.bat` の末尾から常時実行する。
